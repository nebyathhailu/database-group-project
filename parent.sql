create table malnutirition.parent_data(
...
create table malnutirition.parent_child_relation(
relation_id VARCHAR (20) primary key,
parent_id VARCHAR (20) references malnutirition.parent_data(parent_id),
child_id VARCHAR (20) references malnutirition.child_data(child_id)
);


insert into malnutirition.parent_data(parent_id, parent_name, gender, home_address, phone_number)
...
insert into malnutirition.parent_child_relation(relation_id, parent_id, child_id)
values ('R01','P01','020'),
('R02','P02','021'),
('R03','P03','022'),
('R04','P04','023'),
...
--children who have more than 1 parent
select count (*)
FROM(
SELECT child_id
from malnutirition.parent_child_relation
group by child_id 
having COUNT(parent_id)>1
) as children_with_many_parent;


-- parent whose name strats with "K" or 'k'
select count (*)
from malnutirition.parent_data 
